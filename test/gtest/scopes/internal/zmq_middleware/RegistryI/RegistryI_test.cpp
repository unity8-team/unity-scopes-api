...
TEST(RegistryI, get_metadata)
{
    ...
    RegistryObject::SPtr ro(make_shared<RegistryObject>(*scope.death_observer, executor, middleware, "dummy"));
...
public:
    MockRegistryObject(core::posix::ChildProcess::DeathObserver& death_observer)
        : RegistryObject(death_observer, make_shared<Executor>(), nullptr, "dummy")
    {
    }
...
public:
    RegistryITest()
    {
        ...
        Executor::SPtr executor = make_shared<Executor>();
        reg = RegistryObject::SPtr(new RegistryObject(*scope.death_observer, executor, mw, "dummy"));
